...
use spacecraft_core::{Addr, Coin, Timestamp, Uint128};
// we don't import Model and DebugLog in order to use their names for Python classes
use pyo3::{exceptions::PyRuntimeError, prelude::*};

#[pyclass]
struct Model {
    inner: spacecraft_core::Model,
}

#[pyclass]
struct DebugLog {
    inner: spacecraft_core::DebugLog,
}
...
impl Model {
    ...
    fn new(url: String, block_number: Option<u64>, bech32_prefix: String) -> PyResult<Model> {
        let model = spacecraft_core::Model::new(&url, block_number, &bech32_prefix)
...
#[pymodule]
fn spacecraft(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_class::<Model>()?;
